...
weekOrMonth.addEventListener('change', () => {
...
var update_displayed_range = () => {
    ...
    var monthEndDate = getLastDateOfMonth(monthStartDate);
...
async function fetchPhoneData(){
    try {
        const response = await axios.get("https://my-api-name-one.vercel.app/api/v1/number")
...
function update_chart(){
    ...
    // organized into the number of days
    var relevantPhoneData = []

    // only gathering data that is within the interval
    if (isWeeks){
        for (let i = 0; i < 7; i++) relevantPhoneData[i] = 0;
        for (let i = 0; i < 7; i++){
            for (let j = 0; j < phoneData.length; j++){
                let L = phoneData[j][0], R = phoneData[j][1];

                let amt = Math.min(R, weekStartDate + (i+1) * (1000*60*60*7)-1) - Math.max(L, weekStartDate + i * (1000*60*60*7))
                relevantPhoneData[i] += amt;
            }
        }
    } else {
        for (let i = 0; i < monthDays[monthStartDate.getMonth()]; i++) relevantPhoneData[i] = 0;
        for (let i = 0; i < monthDays[monthStartDate.getMonth()]; i++){
            for (let j = 0; j < phoneData.length; j++){
                let L = phoneData[j][0], R = phoneData[j][1];

                let amt = Math.min(R, monthStartDate + (i+1) * (1000*60*60*7)-1) - Math.max(L, monthStartDate + i * (1000*60*60*7))
                relevantPhoneData[i] += amt;
            }
        }
    } 
            ...
            datasets: [
                {
                    ...
                    data: relevantPhoneData.map((row, index) => ({ x: index, y: row }))
                },
                {
        ...
        options: {
            scales: {
                x: {
                    ...
                    labels: relevantPhoneData.map((row, index) => (formatShortDate( new Date(
                            (new Date(isWeeks ? weekStartDate : monthStartDate)).getTime() + index * (1000 * 60 * 60 * 24) 
                        ) 
